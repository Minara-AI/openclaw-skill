#!/usr/bin/env bash
# One-click install & update: minara-cli + minara skill for Claude Code
...
# ---------------------------------------------------------------------------
# Hardcoded content block for ~/.claude/CLAUDE.md
# ---------------------------------------------------------------------------
CLAUDE_MD_BLOCK='## Minara — finance & markets skill
...
# ---------------------------------------------------------------------------
# Inject or update Minara section in ~/.claude/CLAUDE.md
# ---------------------------------------------------------------------------
_inject_claude_md() {
  mkdir -p "$HOME/.claude"

  if [[ -f "$CLAUDE_MD" ]] && grep -q "## Minara" "$CLAUDE_MD" 2>/dev/null; then
    local start_line next_section total_lines end_line
    start_line=$(grep -n "## Minara" "$CLAUDE_MD" | head -1 | cut -d: -f1)
    total_lines=$(wc -l < "$CLAUDE_MD" | tr -d ' ')
    next_section=$(tail -n +"$((start_line + 1))" "$CLAUDE_MD" | grep -n "^## " | head -1 | cut -d: -f1 || true)
    ...
    local tmp_claude="$TMP_DIR/claude_new.md"
    if [[ "$start_line" -gt 1 ]]; then
      head -n "$((start_line - 1))" "$CLAUDE_MD" > "$tmp_claude"
    else
      : > "$tmp_claude"
    fi
    ...
    echo "    Minara config updated in CLAUDE.md"
    return 0
  fi
  ...
  printf '\n\n%s\n' "$CLAUDE_MD_BLOCK" >> "$CLAUDE_MD"
  echo "    Minara config applied to CLAUDE.md"
}

# ===== MAIN =====

echo "==> Minara setup for Claude Code"
echo ""
